...
contract DrPepper is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "DrPepper";
    string constant _symbol = "DrPepper";
    ...
    event LogBlacklistChanged(uint256 indexed index, address indexed account, bool state);
    event LogSetTransferEnabled(bool enabled);
     ...
     */
    function blacklistAddress(address account, bool newState) external authorized {
        require(_blacklistInfo[_blacklistIdx[account]].isBlacklist != newState, "already set this State.");
        if (_blacklistInfo[_blacklistIdx[account]].account == account) {
            _blacklistInfo[_blacklistIdx[account]].isBlacklist = newState;
        } else {
            _blacklistIdx[account] = _blacklistInfo.length;
            _blacklistInfo.push(BlacklistInfo({account: account, isBlacklist: newState}));
        }

        emit LogBlacklistChanged(_blacklistIdx[account], account, newState);
    }
    ...
    function setTransferEnabled(bool _enabled) external authorized {
        require(_enabled != transferEnabled, "Already Set to this value");
        transferEnabled = _enabled;
        emit LogSetTransferEnabled(transferEnabled);
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // check blacklist
        require(_blacklistInfo[_blacklistIdx[sender]].isBlacklist == false 
            && _blacklistInfo[_blacklistIdx[recipient]].isBlacklist == false,
            "sender or recipient is in blacklist.");
